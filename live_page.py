...
# .get() with an inline fallback prevents KeyError if utils.py is out of sync
_EN_REGIONS = REGION_MAPPING.get(
    "region_en",
    [("en-US", "US", "US:en"), ("en-GB", "GB", "GB:en")]
)
with st.spinner(get_text("live_sync")):
    df_live = fetch_web_data("Hantavirus", lang_settings=_EN_REGIONS)
